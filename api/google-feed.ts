...
function sanitizeClaim(text: string): string {
  return text
    ...
    .replace(/\bиммунитет|immune(?:\s|-)?boost/gi, 'защитных сил')
    .replace(/\bпохуден|weight\s?(?:loss|control)|снижени\w* веса?/gi, 'контроля веса')
    .replace(/\bанеми[яи]|anemia/gi, 'дефицита железа')
    .replace(/\bгемоглобин|hemoglobin/gi, 'крови')
    .replace(/\bдефицит[а-я]*|deficiency/gi, 'недостатка')
    .replace(/\s+/g, ' ')
    .trim();
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  xml += '<link>' + SITE + '</link>\n';
  xml += '<description>Оригинальные витамины и БАДы из Турции от бренда Daruzen. Натуральные добавки для здоровья, энергии и красоты.</description>\n';

  const RATE_RUB = 2.2;

  for (const p of products) {
    const name = p.name_ru || p.name_en || p.name_ru || p.id;
    if (!name) continue;
    const rawDesc = getDesc(p, 'ru') || getDesc(p, 'en');
    const desc = sanitizeClaim(rawDesc);
    const imageUrl = p.image?.startsWith('http') ? p.image : SITE + p.image;
    const basePrice = Number(p.price) || 0;
    const priceVal = (basePrice * RATE_RUB).toFixed(2);
    ...
    xml += '<g:price>' + priceVal + ' RUB</g:price>\n';
